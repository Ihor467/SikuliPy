"""X11 keysym constants — port of ``org.sikuli.vnc.XKeySym`` (auto-generated).

Generated from Oculix API/src/main/java/org/sikuli/vnc/XKeySym.java. Do not
edit by hand; rerun the extractor in tools/ if this list needs to drift.

Use :func:`keysym_name` to round-trip an integer keysym back to its
canonical X11 name (first definition wins for aliased keysyms).
"""
...
# Reverse mapping — name lookup for a keysym integer. First-defined name wins.
_NAME_BY_KEYSYM: dict[int, str] = {}
for _n, _v in list(globals().items()):
    if _n.startswith("XK_") and isinstance(_v, int) and _v not in _NAME_BY_KEYSYM:
        _NAME_BY_KEYSYM[_v] = _n[3:]
del _n, _v


def keysym_name(keysym: int) -> str | None:
    """Return the X11 keysym *name* for ``keysym`` (e.g. 0xff0d → ``"Return"``).
    
    Returns ``None`` if the keysym is not one of the known constants.
    """
